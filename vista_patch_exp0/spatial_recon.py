...
filepaths = glob.glob(prefix+'/dataset/france2/processed_lai_npy2/*.tif')
filepaths.sort()
...
def evaluate_months(season, ulx, uly, pixel_size, crs_code):
    ...
    labels = np.load(prefix+'/storage/full_mast1/vista_labes_aligned.npy').astype(np.uint8)
    ...
    for filepath in considered_filepaths:
        #numpy_array = np.load(filepath, allow_pickle=True).astype(np.float32)
        numpy_array = imread(filepath).astype(np.float32)
    ...
    for test_img_number in range(len(X_test)):
        ...
        for chosen_crop_types_list in chosen_crop_types_list_list_models:
            ...
            my_model_1.load_weights(prefix+'/checkpoints_f2/3D_unet_g4_h4_crop_'+str(chosen_crop_types_list[0]-10)+'_'+str(chosen_crop_types_list[1]-10)+'_'+str(chosen_crop_types_list[2]-10)+'_epoch_f.h5', by_name=True, skip_mismatch=True)
